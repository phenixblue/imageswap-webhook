...
run_resource_tests() {
  ...
  resource=$(yq --stripComments "${TESTS_MANIFEST}" "resources.[${resource_index}].kind")

  # grab local test type from ${TESTS_MANIFEST}
  local test_type
  
  test_type=$(yq --stripComments "${TESTS_MANIFEST}" "resources.[${resource_index}].desired")

  # grab local list of test manifests to use
  local manifest_list
  
  manifest_list=$(yq --stripComments -P "${TESTS_MANIFEST}" "resources.[${resource_index}].manifests" | sed 's/^-[ ]*//')

  # grab local user_script specified for pre/post/between running
  local user_script
  
  user_script=$(yq --stripComments -P "${TESTS_MANIFEST}" "resources.[${resource_index}].script")
...
scope_and_run_tests() {
  ...
  resource_array_length=$(yq --stripComments -l "${TESTS_MANIFEST}" 'resources')
  ...
  for ((i = 0 ; i < resource_array_length ; i++)); do
    ...
    if [[ "${TEST_RESOURCE_KIND}" == "all" ]] || [[ "${TEST_RESOURCE_KIND}" == "$(yq --stripComments "${TESTS_MANIFEST}" "resources.[${i}].kind")" ]]; then

      # check if we're doing all desired results or if the resoured desired result at $i matches the requested desired result
      if [[ "${TEST_RESOURCE_DESIRED}" == "all" ]] || [[ "${TEST_RESOURCE_DESIRED}" == "$(yq --stripComments "${TESTS_MANIFEST}" "resources.[${i}].desired")" ]]; then
